...
import com.inforegistru.levelstat.Helpers.MyAdapterclprodmold;
...
import com.inforegistru.levelstat.Retrofit.ResponseModelCl_prodmold;
...
public class ScientistsActivityclprodmold extends AppCompatActivity
    ...
    private void retrieveAndFillRecyclerView(final String action, String queryString,
        ...
        if (action.equalsIgnoreCase("GET_PAGINATEDCLPROD")) {
            retrievedData = api.search_cl_prodmold("GET_PAGINATEDCLPROD", queryString, start, limit);
            Utils.showProgressBar(mProgressBar);
        } else if (action.equalsIgnoreCase("GET_PAGINATED_SEARCHCLPROD")) {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.search_cl_prodmold("GET_PAGINATED_SEARCHCLPROD", queryString, start, limit);
        } else {
            Utils.showProgressBar(mProgressBar);
        ...
        retrievedData.enqueue(new Callback<ResponseModelCl_prodmold>() {
            @Override
            public void onResponse(Call<ResponseModelCl_prodmold> call, Response<ResponseModelCl_prodmold>
                    response) {
            ...
            @Override
            public void onFailure(Call<ResponseModelCl_prodmold> call, Throwable t) {
                Utils.hideProgressBar(mProgressBar);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_new_cl_prodmold:
                Utils.openActivity(this, help_vw.class);
                finish();
                return true;

            case R.id.action_new_en_cl_prodmold:
                Utils.openActivity(this, help_vw_en.class);
                finish();
                return true;


            case R.id.action_new_ru_cl_prodmold:
...
//    }
    ...
    public boolean onQueryTextChange(String query) {
        retrieveAndFillRecyclerView("GET_PAGINATED_SEARCHCLPROD", query, "0", "100");
        return false;
    }
...
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupRecyclerView();
        retrieveAndFillRecyclerView("GET_PAGINATEDCLPROD", "", "0", "100");
    }
}
