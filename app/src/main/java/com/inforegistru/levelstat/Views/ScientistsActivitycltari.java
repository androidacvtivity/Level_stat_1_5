...
import com.inforegistru.levelstat.Helpers.MyAdaptercltari;
...
import com.inforegistru.levelstat.Retrofit.ResponseModelCl_tari;
...
public class ScientistsActivitycltari extends AppCompatActivity
    ...
    private void retrieveAndFillRecyclerView(final String action, String queryString,
        ...
        if (action.equalsIgnoreCase("GET_PAGINATEDCLTARI")) {
            retrievedData = api.search_cl_tari("GET_PAGINATEDCLTARI", queryString, start, limit);
            Utils.showProgressBar(mProgressBar);
        } else if (action.equalsIgnoreCase("GET_PAGINATED_SEARCLTARI")) {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.search_cl_tari("GET_PAGINATED_SEARCLTARI", queryString, start, limit);
        } else {
            Utils.showProgressBar(mProgressBar);
        ...
        retrievedData.enqueue(new Callback<ResponseModelCl_tari>() {
            @Override
            public void onResponse(Call<ResponseModelCl_tari> call, Response<ResponseModelCl_tari>
                    response) {
            ...
            @Override
            public void onFailure(Call<ResponseModelCl_tari> call, Throwable t) {
                Utils.hideProgressBar(mProgressBar);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_new_cl_tari:
                Utils.openActivity(this, help_vw.class);
                finish();
                return true;

            case R.id.action_new_en_cl_tari:
                Utils.openActivity(this, help_vw_en.class);
                finish();
                return true;


            case R.id.action_new_ru_cl_tari:
...
//    }
    ...
    public boolean onQueryTextChange(String query) {
        retrieveAndFillRecyclerView("GET_PAGINATED_SEARCLTARI", query, "0", "100");
        return false;
    }
...
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupRecyclerView();
        retrieveAndFillRecyclerView("GET_PAGINATEDCLTARI", "", "0", "100");
    }
}
