...
import com.inforegistru.levelstat.Helpers.MyAdapterclcfp;

import com.inforegistru.levelstat.Helpers.Utils;
import com.inforegistru.levelstat.R;


import com.inforegistru.levelstat.Retrofit.ResponseModelCl_cfp;
...
public class ScientistsActivityclcfp extends AppCompatActivity
    ...
    private void retrieveAndFillRecyclerView(final String action, String queryString,
        ...
        if (action.equalsIgnoreCase("GET_PAGINATEDCFP")) {
            retrievedData = api.search_cl_cfp("GET_PAGINATEDCFP", queryString, start, limit);
            Utils.showProgressBar(mProgressBar);
        } else if (action.equalsIgnoreCase("GET_PAGINATED_SEARCHCFP")) {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.search_cl_cfp("GET_PAGINATED_SEARCHCFP", queryString, start, limit);
        } else {
            Utils.showProgressBar(mProgressBar);
        ...
        retrievedData.enqueue(new Callback<ResponseModelCl_cfp>() {
            @Override
            public void onResponse(Call<ResponseModelCl_cfp> call, Response<ResponseModelCl_cfp>
                    response) {
            ...
            @Override
            public void onFailure(Call<ResponseModelCl_cfp> call, Throwable t) {
                Utils.hideProgressBar(mProgressBar);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_new_cl_cfp:
                Utils.openActivity(this, help_vw.class);
                finish();
                return true;

            case R.id.action_new_en_cl_cfp:
                Utils.openActivity(this, help_vw_en.class);
                finish();
                return true;


            case R.id.action_new_ru_cl_cfp:
