...
import com.inforegistru.levelstat.Helpers.MyAdaptercu;
import com.inforegistru.levelstat.Helpers.Utils;
import com.inforegistru.levelstat.R;
import com.inforegistru.levelstat.Retrofit.ResponseModelcu;
...
public class ScientistsActivitycu extends AppCompatActivity
    ...
    private void retrieveAndFillRecyclerView(final String action, String queryString,
        ...
        if (action.equalsIgnoreCase("GET_PAGINATEDCU")) {
            retrievedData = api.searchcu("GET_PAGINATEDCU", queryString, start, limit);
            Utils.showProgressBar(mProgressBar);
        } else if (action.equalsIgnoreCase("GET_PAGINATED_SEARCHCU")) {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.searchcu("GET_PAGINATED_SEARCHCU", queryString, start, limit);
        } else {
            Utils.showProgressBar(mProgressBar);
        ...
        retrievedData.enqueue(new Callback<ResponseModelcu>() {
            @Override
            public void onResponse(Call<ResponseModelcu> call, Response<ResponseModelcu>
                    response) {
            ...
            @Override
            public void onFailure(Call<ResponseModelcu> call, Throwable t) {
                Utils.hideProgressBar(mProgressBar);
...
//    }
    ...
    public boolean onQueryTextChange(String query) {
        retrieveAndFillRecyclerView("GET_PAGINATED_SEARCHCU", query, "0", "100");
        return false;
    }
...
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupRecyclerView();
        retrieveAndFillRecyclerView("GET_PAGINATEDCU", "", "0", "100");
    }
}