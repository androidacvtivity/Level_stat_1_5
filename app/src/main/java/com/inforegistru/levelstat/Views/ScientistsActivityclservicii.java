...
import com.inforegistru.levelstat.Helpers.MyAdapterclcspm;
...
import com.inforegistru.levelstat.Retrofit.ResponseModelCl_servicii;
...
public class ScientistsActivityclservicii extends AppCompatActivity
    ...
    private void retrieveAndFillRecyclerView(final String action, String queryString,
        ...
        if (action.equalsIgnoreCase("GET_PAGINATEDCSERVICII")) {
            retrievedData = api.search_cl_servicii("GET_PAGINATEDCSERVICII", queryString, start, limit);
            Utils.showProgressBar(mProgressBar);
        } else if (action.equalsIgnoreCase("GET_PAGINATED_SEARCHERVICII")) {
            Utils.showProgressBar(mProgressBar);
            retrievedData = api.search_cl_servicii("GET_PAGINATED_SEARCHERVICII", queryString, start, limit);
        } else {
            Utils.showProgressBar(mProgressBar);
        ...
        retrievedData.enqueue(new Callback<ResponseModelCl_servicii>() {
            @Override
            public void onResponse(Call<ResponseModelCl_servicii> call, Response<ResponseModelCl_servicii>
                    response) {
            ...
            @Override
            public void onFailure(Call<ResponseModelCl_servicii> call, Throwable t) {
                Utils.hideProgressBar(mProgressBar);
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.action_new_cl_servicii:
                Utils.openActivity(this, help_vw.class);
                finish();
                return true;

            case R.id.action_new_en_cl_servicii:
                Utils.openActivity(this, help_vw_en.class);
                finish();
                return true;


            case R.id.action_new_ru_cl_servicii:
...
//    }
    ...
    public boolean onQueryTextChange(String query) {
        retrieveAndFillRecyclerView("GET_PAGINATED_SEARCHERVICII", query, "0", "100");
        return false;
    }
...
//    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setupRecyclerView();
        retrieveAndFillRecyclerView("GET_PAGINATEDCSERVICII", "", "0", "100");
    }
}
